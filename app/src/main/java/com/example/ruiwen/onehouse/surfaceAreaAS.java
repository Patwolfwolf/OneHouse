...
public class surfaceAreaAS extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_surface_area_as);
        ...
        calBut.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(area.length() == 0&& radius.length() != 0 ){
                    area.setText(valueOf(geo.getSurfaceAS(valueOf(radius.getText().toString()))).toString());
                }
                if(radius.length() == 0&& area.length() != 0 ){
                    radius.setText(valueOf(geo.getRadiusSS(valueOf(area.getText().toString()))).toString());
                }
            }
        });
    }
